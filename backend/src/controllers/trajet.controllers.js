const { getTrajet, getTrajets } = require('../models/trajet.model');
const { getTrajetsStatsDetails } = require('../utils/formatDataTrajets')

async function httpGetTrajet(req, res) {
    try {
        return res.status(200).json(await getTrajet());
    } catch (error) {
        console.log(error);
...
async function httpGetTrajetsByParams(req, res) {
    try {
        ...
        const results = await getTrajets({
            dateFrom,
            dateTo,
        ...
        const data = await getTrajetsStatsDetails(results.data);

        return res.status(200).json(data);
    } catch (error) {
        console.error('Erreur dans httpGetTrajetsByParams:', error);
...
module.exports = { httpGetTrajet, httpGetTrajetsByParams }